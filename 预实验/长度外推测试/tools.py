...
class LossRecorder:
    ...
    mean_loss=-1.0
    history_decay=0.9
    new_loss_weight=1-history_decay
    train_loss_line_per_epoch = []
    test_loss_line_per_epoch = []
    train_loss_line = []
    test_loss_line = []
    mean_loss_line = []
    
    @classmethod
    def update_mean_loss_line(cls, new_loss):
        "记录损失函数值"
        if cls.mean_loss < 0.0:
            cls.mean_loss = new_loss
        else:
            cls.mean_loss *= cls.history_decay
            cls.mean_loss += new_loss * cls.new_loss_weight
            cls.mean_loss_line += [cls.mean_loss]
